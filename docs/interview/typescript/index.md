...
## TypeScript中的枚举
* 枚举就是定义一些带名字的常量，可以清晰地表达意图或区别的用例。
* ts中枚举的值可以是数字或字符串。

## TypeScript 的主要特点
* 跨平台：typescript 编译器可以安装到任何操作系统上
* ES6 特性：ts 包含计划中的 ES6的大部分特性
* 面向对象语言：提供标准的 OOP 功能
* 静态类型检查：编译时进行类型检查
* 可选静态类型
* DOM操作：使用 ts 来操作 DOM 已添加或删除客户端网页元素

## TypeScript中的方法重写
* 与 Java 的重写一样，重写发生在父子类中
* 方法名称、参数必须跟父类一致
* 返回类型可以不一致，但必须是父类返回值的派生类